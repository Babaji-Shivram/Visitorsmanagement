...
namespace VisitorManagement.API.Services
{
    public class AuthService : IAuthService
    {
        ...
        public async Task<LoginResponseDto?> StaffLoginAsync(LoginRequestDto request)
        {
            var staffMember = await _context.StaffMembers
                ...
                .FirstOrDefaultAsync(s => s.Email.ToLower() == request.Email.ToLower() 
                    && s.IsActive && s.CanLogin);

            if (staffMember == null || string.IsNullOrEmpty(staffMember.Password))
                return null;

            // Simple password comparison (in production, use proper hashing)
            if (staffMember.Password != request.Password)
                return null;

            // Create a UserDto from staff member for token generation
            var userDto = new UserDto
            {
                ...
                PhoneNumber = staffMember.MobileNumber,
                Extension = staffMember.Extension,
                Role = staffMember.Role.ToLower() switch
                {
                    "admin" => UserRole.Admin,
                    "reception" => UserRole.Reception,
                    _ => UserRole.Staff
                },
                Department = staffMember.Designation,
                IsActive = staffMember.IsActive,
                LocationId = staffMember.LocationId,
                LocationName = staffMember.Location?.Name
            };
        ...
        public string GenerateJwtToken(UserDto user)
        {
            ...
            // Add location claims if user has a location
            ...
            {
                claims.Add(new Claim("LocationName", user.LocationName));
            }
